...
// Function to fetch params as a Promise
...
export default function UpdateAttractionPage({ params: paramsPromise }: { params: Promise<{ id: string }> }) {
  ...
  // Then fetch the attraction data once we have the params
  useEffect(() => {
    if (!params) return;
    ...
    const fetchAttractionData = async () => {
      setLoadingData(true);
      try {
        // Using Promise to fetch attraction data
        const attraction = await new Promise<typeof dummyAttractions[0] | undefined>((resolve) => {
          setTimeout(() => {
            const found = dummyAttractions.find(a => a.id === params.id);
            resolve(found);
          }, 500);
        });
