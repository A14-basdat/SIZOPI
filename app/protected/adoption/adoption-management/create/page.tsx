...
export default function CreateAdoptionPage() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      tgl_berhenti_adopsi: new Date(new Date().setMonth(new Date().getMonth() + 6)), // Default to 6 months
    ...
    async function fetchData() {
      ...
      try {
        ...
        const { data: animalsData, error: animalsError } = await supabase
          .from('hewan')
          .select('id_hewan, nama_hewan, jenis_spesies, foto')
          .eq('status_adopsi', 'Belum Diadopsi');
        ...
        const enhancedAdopters = await Promise.all((adoptersData || []).map(async (adopter) => {
                        ...
                        <SelectContent>
                          {animals.length === 0 ? (
                            ...
                            animals.map(animal => (
                              <SelectItem 
                                key={animal.id_hewan} 
                                value={animal.id_hewan}
                              >
                                {animal.nama_hewan} ({animal.jenis_spesies})
                              </SelectItem>
                            ))
                          )}
                        </SelectContent>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                        ...
                        <SelectContent>
                          {adopters.length === 0 ? (
                            ...
                            adopters.map(adopter => (
                              ...
                              >
                                {adopter.displayName} ({adopter.jenis_adopter === 'individu' ? 'Individual' : 'Organization'})
                              </SelectItem>
                            ))
                          )}
                        </SelectContent>
                      ...
                      <FormDescription>
                        Select the person or organization that will adopt the animal.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
